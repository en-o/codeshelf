...
2. **安全边界与错误对象操作**：~~AUD-002～AUD-006、AUD-009~~ 已完成，AUD-007 部分完成；
   剩 AUD-008、AUD-016～AUD-020、AUD-045～AUD-050。
3. **数据、升级与发布 P1**：AUD-010、AUD-021～AUD-025、AUD-051。
4. **P2/P3 整理**：其余条目按模块分批处理。

> 进度：11 / 61 已整改（AUD-001、002、003、004、005、006、009、011、039、044），
> AUD-007 部分整改。
...
>
> **发版必须写进 release note 的行为变更**：AUD-009 把端口转发 / SSH 隧道本地口 / 静态服务
> 的默认监听从 `0.0.0.0` 改成 `127.0.0.1`，依赖局域网访问的用户需要手动勾选「对局域网开放」。
>
